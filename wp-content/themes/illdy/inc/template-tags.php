...
if(!function_exists('illdy_archive_title')) {
	function illdy_archive_title( $before = '', $after = '' ) {
		if ( is_category() ) {
			$title = sprintf( __( 'Category: %s', 'illdy' ), single_cat_title( '', false ) );
		} elseif ( is_tag() ) {
			$title = sprintf( __( 'Tag: %s', 'illdy' ), single_tag_title( '', false ) );
		} elseif ( is_author() ) {
			$title = sprintf( __( 'Author: %s', 'illdy' ), '<span class="vcard">' . get_the_author() . '</span>' );
		} elseif ( is_year() ) {
			$title = sprintf( __( 'Year: %s', 'illdy' ), get_the_date( esc_html_x( 'Y', 'yearly archives date format', 'illdy' ) ) );
		} elseif ( is_month() ) {
			$title = sprintf( __( 'Month: %s', 'illdy' ), get_the_date( esc_html_x( 'F Y', 'monthly archives date format', 'illdy' ) ) );
		} elseif ( is_day() ) {
			$title = sprintf( __( 'Day: %s', 'illdy' ), get_the_date( esc_html_x( 'F j, Y', 'daily archives date format', 'illdy' ) ) );
			...
			$title = sprintf( __( 'Archives: %s', 'illdy' ), post_type_archive_title( '', false ) );
		} elseif ( is_tax() ) {
			$tax = get_taxonomy( get_queried_object()->taxonomy );
			/* translators: 1: Taxonomy singular name, 2: Current taxonomy term */
			$title = sprintf( __( '%1$s: %2$s', 'illdy' ), $tax->labels->singular_name, single_term_title( '', false ) );
		} else {
			$title = __( 'Archives', 'illdy' );
		}
